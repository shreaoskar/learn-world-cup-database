...
echo "$($PSQL "SELECT teams.name FROM teams FULL JOIN teams_games ON teams.team_id = teams_games.teams_id FULL JOIN games ON teams_games.games_id = games.game_id WHERE year = 2014 AND round = 'Eighth-Final' ORDER BY teams.name")"

echo -e "\nList of unique winning team names in the whole data set:"
echo "$($PSQL "SELECT DISTINCT(teams.name) FROM games INNER JOIN teams ON games.winner_id = teams.team_id WHERE games.winner_id = teams.team_id ORDER BY teams.name")"
